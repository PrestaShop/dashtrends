...
class dashtrends extends Module
{
    ...
    public function hookDisplayAdminDashboardZoneTwo(array $params)
    {
        ...
        $dashboardData = $this->refineData($params['date_from'], $params['date_to'], $tmp_data);

        return $this->render('zone_two.html.twig', [
            'title' => $this->trans('Sales trend', [], 'Modules.Dashtrends.Admin'),
            'chartId' => 'dashtrends-sales',
            'chartConfig' => $this->getSalesChartConfig($dashboardData),
        ]);
    }
     ...
     */
    private function getSalesChartConfig(array $dashboardData): array
    {
        $labels = [];
        $data = [];
        foreach ($dashboardData['sales'] as $timestamp => $value) {
